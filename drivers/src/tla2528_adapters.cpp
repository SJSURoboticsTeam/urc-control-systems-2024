...
tla2528_output_pin::tla2528_output_pin(tla2528& p_tla2528, hal::byte p_channel, hal::output_pin::settings const& p_settings) {
    if (hal::bit_extract(hal::bit_mask::from(p_channel),p_tla2528.m_object_created)) throw hal::resource_unavailable_try_again(this);
    hal::bit_modify(p_tla2528.m_object_created).set(hal::bit_mask::from(p_channel));
    m_tla2528 = &p_tla2528;
    m_channel = p_channel;
    driver_configure(p_settings);
}
tla2528_output_pin::~tla2528_output_pin(){
    hal::bit_modify(m_tla2528->m_object_created).set(hal::bit_mask::from(m_channel));
}
