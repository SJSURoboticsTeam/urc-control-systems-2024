...
sht21::sht21(hal::i2c& p_i2c, hal::byte p_address) : m_i2c(p_i2c), m_address(p_address) {}
      
void sht21::soft_reset() {
  hal::write(m_i2c, m_address, std::array<hal::byte, 1> { command::soft_reset_command });
}

bool sht21::is_low_battery() {
  std::array<hal::byte, 1> response_buffer;
  hal::write_then_read(m_i2c, m_address, std::array<hal::byte, 1> { command::read_user_register_command }, response_buffer);
...
void sht21::set_resolution(resolution p_resolution) {
  ...
  hal::write(m_i2c, m_address, std::array<hal::byte, 2> { command::write_user_register_command, response_buffer[0] } );
}
