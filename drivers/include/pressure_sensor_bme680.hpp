...
public:
  ...
  void print_calibration_coefficients(hal::v5::strong_ptr<hal::serial> console);

  // /// @brief Print a register. For debugging
  // /// @param console Console to print to.
  // /// @param register_address Register to read
  // /// @param format Format to print register as.
  // void print_register(hal::v5::strong_ptr<hal::serial> console, hal::byte
  // register_address, format format = format::hex);

  /// @brief Construct a 
  /// @param p_i2c bus
  /// @param p_address address
  explicit bme680(hal::v5::strong_ptr<hal::i2c> p_i2c, hal::byte p_address);
