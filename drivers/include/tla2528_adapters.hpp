...
{
private:
  tla2528_output_pin(tla2528& p_tla2528, hal::byte p_channel, hal::output_pin::settings const& p_settings);
  void driver_configure(settings const& p_settings);
  void driver_level(bool p_high);
  bool driver_level();
  friend tla2528_output_pin make_output_pin(tla2528& p_tla2528, hal::byte p_channel, hal::output_pin::settings const& p_settings);
  tla2528* m_tla2528 = nullptr;
  hal::byte m_channel;
public:
  ~tla2528_output_pin();
};
tla2528_output_pin make_output_pin(tla2528& p_tla2528, hal::byte p_channel, hal::output_pin::settings const& p_settings);
